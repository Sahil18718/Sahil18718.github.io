...
const Experience = () => {
  return (
    <section id="experience" className="py-16 md:py-24 px-4 sm:px-6 bg-gray-50 dark:bg-black">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12 md:mb-16">
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-4">Professional Experience</h2>
        ...
        <div className="space-y-8 md:space-y-12">
          {experiences.map((exp, index) => (
            <div
              key={index}
              className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 sm:p-8 hover:shadow-lg dark:hover:shadow-2xl dark:hover:shadow-pink-500/10 transition-all duration-200"
            >
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6">
                <div className="mb-4 sm:mb-0">
                  <h3 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white mb-2">{exp.role}</h3>
                  <div className="flex flex-col sm:flex-row sm:items-center text-gray-600 dark:text-gray-300 mb-2 gap-1 sm:gap-0">
                    ...
                    <span className="bg-pink-100 dark:bg-pink-900/50 text-pink-800 dark:text-pink-300 px-2 py-1 rounded text-sm w-fit">
                      {exp.type}
                    </span>
                  </div>
              ...
              <div className="mb-6">
                <ul className="space-y-2">
                  {exp.responsibilities.map((resp, i) => (
                    <li
                      key={i}
                      className="text-gray-600 dark:text-gray-300 leading-relaxed flex items-start text-sm sm:text-base"
                    >
                      <span className="w-1.5 h-1.5 bg-pink-500 dark:bg-pink-400 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                      {resp}
                    </li>
                  ))}
                </ul>
